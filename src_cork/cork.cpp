...
namespace corkpp {
  ...
  struct CorkVertex : public MinimalVertexData,
  ...
  struct CorkTriangle : public MinimalTriangleData,
  ...
  void corkTriMesh2CorkMesh(CorkTriMesh in, CorkMesh & mesh_out) {
    ...
    for (uint i = 0; i < in.n_triangles; i++) {
      ...
      max_ref_idx =
          std::max(std::max(max_ref_idx, in.triangles[3 * i + 0]),
                   std::max(in.triangles[3 * i + 1], in.triangles[3 * i + 2]));
    }
  ...
  void computeIntersection(CorkTriMesh in0, CorkTriMesh in1,
                           CorkTriMesh & out) {
    CorkMesh cmIn0, cmIn1;
    corkTriMesh2CorkMesh(in0, cmIn0);
    corkTriMesh2CorkMesh(in1, cmIn1);
  ...
  void computeSymmetricDifference(CorkTriMesh in0, CorkTriMesh in1,
                                  CorkTriMesh & out) {
    CorkMesh cmIn0, cmIn1;
    corkTriMesh2CorkMesh(in0, cmIn0);
    corkTriMesh2CorkMesh(in1, cmIn1);
  ...
  void resolveIntersections(CorkTriMesh in0, CorkTriMesh in1,
                            CorkTriMesh & out) {
    CorkMesh cmIn0, cmIn1;
    corkTriMesh2CorkMesh(in0, cmIn0);
    corkTriMesh2CorkMesh(in1, cmIn1);
