...
namespace corkpp {
  namespace Files {
    ...
    inline bool read_vertex(ifstream & in, Vec3d & data) {
      Vec3f temp;
      bool result = read_float32(in, temp.x) && read_float32(in, temp.y) &&
    ...
    inline bool write_vertex(ofstream & out, const Vec3d & data) {
      ...
      bool result = write_float32(out, data.x) && write_float32(out, data.y) &&
    ...
    inline bool write_triangle(ofstream & out, const FileTriangle & data) {
      return write_uint32(out, data.a) && write_uint32(out, data.b) &&
    ...
    int readIFS(string filename, FileMesh * data) {
      ...
      if (!readString(in, junk_str))
        return 1;

      // version #
      float32 version;
      bool hasTextures;
      if (!read_float32(in, version))
        return 1;
      if (version == 1.0) {
        hasTextures = false;
      } else if (version == 1.1) {
      ...
      // VERTICES
      if (!checkString(in, "VERTICES"))
        return 1;
      if (!readString(in, junk_str))
        return 1;
      uint32 num_vertices;
      if (!read_uint32(in, num_vertices))
        return 1;
      data->vertices.resize(num_vertices);
      for (auto & v : data->vertices) {
        if (!read_vertex(in, v.pos))
          return 1;
      }

      // TRIANGLES
      if (!checkString(in, "TRIANGLES"))
        return 1;
      if (!readString(in, junk_str))
        return 1;
      uint32 num_tris;
      if (!read_uint32(in, num_tris))
        return 1;
      data->triangles.resize(num_tris);
      for (auto & tri : data->triangles) {
        if (!read_triangle(in, tri))
          return 1;
      }
    ...
    int writeIFS(string filename, FileMesh * data) {
      ...
      // "IFS"
      if (!writeString(out, "IFS"))
        return 1;
      // version #
      float32 version = 1.0;  // i.e. no textures
      if (!write_float32(out, version))
        return 1;
      // model name
      if (!writeString(out, "no_name"))
        return 1;

      // VERTICES
      if (!writeString(out, "VERTICES"))
        return 1;
      uint32 num_vertices = data->vertices.size();
      if (!write_uint32(out, num_vertices))
        return 1;
      for (const auto & v : data->vertices) {
        if (!write_vertex(out, v.pos))
          return 1;
      }

      // TRIANGLES
      if (!writeString(out, "TRIANGLES"))
        return 1;
      uint32 num_tris = data->triangles.size();
      if (!write_uint32(out, num_tris))
        return 1;
      for (const auto & tri : data->triangles) {
        if (!write_triangle(out, tri))
          return 1;
      }
