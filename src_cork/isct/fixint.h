...
#define SIGN_LIMB(limbs, n)                                                    \
  (((limbs)[(n)-1] & LIMB_SIGN_MASK) ? ONES_PATTERN : ZERO_PATTERN)
#define BITS_TO_LIMBS(n) ((((n)-1) / LIMB_BIT_SIZE) + 1)
    ...
    inline void add(LimbInt<Nout> & out, const LimbInt<Nlhs> & lhs,
      ...
      if (Nlhs == Nrhs) {
        ...
        mp_limb_t borrow = mpn_sub_1(out.limbs + Nrhs, out.limbs + Nrhs,
                                     Nlhs - Nrhs, rhs_is_neg);
        if (Nout > Nmax)
        ...
        mp_limb_t borrow = mpn_sub_1(out.limbs + Nlhs, out.limbs + Nlhs,
                                     Nrhs - Nlhs, lhs_is_neg);
        if (Nout > Nmax)
          carry = carry | (mp_limb_t(1) - borrow);
      }
    ...
    template <int Nout, int Nlhs, int Nrhs>
    inline void sub(LimbInt<Nout> & out, const LimbInt<Nlhs> & lhs,
                    const LimbInt<Nrhs> & rhs) {
      // for testing...
    ...
    template <int Nout, int Nlhs, int Nrhs>
    inline void mul(LimbInt<Nout> & out, const LimbInt<Nlhs> & lhs,
                    const LimbInt<Nrhs> & rhs) {
      ASSERT_STATIC<(Nout >= Nlhs && Nout >= Nrhs)>::test();
    ...
    inline std::string toString(const LimbInt<N> & num) {
      ...
      int count = mpn_get_str(reinterpret_cast<unsigned char *>(cbuf), 10,
                              garbage.limbs, N);
