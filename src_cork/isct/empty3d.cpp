...
namespace corkpp {
  namespace Empty3d {
    ...
    bool isEmpty(const TriEdgeIn & input) {
      ...
      join(temp_e2, p_isct, tp1);
      join(a_t0, temp_e2, tp2);
      join(temp_e2, tp0, p_isct);
      join(a_t1, temp_e2, tp2);
      join(temp_e2, tp0, tp1);
      join(a_t2, temp_e2, p_isct);
      Ext4_2 a_e0, a_e1;
      join(a_e0, p_isct, ep1);
      join(a_e1, ep0, p_isct);

      return (inner(t_ext3, a_t0) < 0.0) || (inner(t_ext3, a_t1) < 0.0) ||
             (inner(t_ext3, a_t2) < 0.0) || (inner(e_ext2, a_e0) < 0.0) ||
    ...
    int emptyFilter(const TriEdgeIn & input) {
      ...
      for (int i = 0; i < 2; i++) {
        Ext4_2 a;
        AbsExt4_2 ka;
        join(a, (i == 0) ? pisct : ep[0], (i == 1) ? pisct : ep[1]);
      ...
      for (int i = 0; i < 3; i++) {
        Ext4_3 a;
        AbsExt4_3 ka;
        join(temp2, (i == 0) ? pisct : tp[0], (i == 1) ? pisct : tp[1]);
        join(a, temp2, (i == 2) ? pisct : tp[2]);
    ...
    bool exactFallback(const TriEdgeIn & input) {
      ...
      if (sign_e0 < 0 || sign_e1 < 0 || sign_t0 < 0 || sign_t1 < 0 ||
          sign_t2 < 0)
        return true;
    ...
    int emptyFilter(const TriTriTriIn & input) {
      ...
      for (uint i = 0; i < 3; i++) {
        for (uint j = 0; j < 3; j++) {
          toExt(p[i][j], input.tri[i].p[j]);
          abs(kp[i][j], p[i][j]);
        }
        join(temp2, p[i][0], p[i][1]);
        join(ktemp2, kp[i][0], kp[i][1]);
        join(t[i], temp2, p[i][2]);
