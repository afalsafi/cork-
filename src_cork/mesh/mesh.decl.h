...
namespace corkpp {
  ...
  struct RemeshOptions {
    ...
    double maxAngle;
    RemeshOptions()
        : maxEdgeLength(1.0), minEdgeLength(0.3), minAngle(5.0),
          maxAngle(170.0) {}
  };
  ...
  class Mesh {
   public:
    ...
    inline void
    for_tris(std::function<void(TriData &, VertData &, VertData &, VertData &)>
                 func);
    ...
    inline void accessIsct(
        const Isct & isct,
        std::function<void(TriData &, VertData &, VertData &, VertData &)>
            func);
   ...
   private:  // caches
    ...
    struct EGraphCache {
      ...
      inline void
      for_each(std::function<void(uint i, uint j, EGraphEntry<Edata> & entry)>
                   action) {
   ...
   private:  // Remeshing Support
    ...
    void edgeCollapse(RemeshScratchpad &, Eptr e_collapse,
  ...
  template <class VertData, class TriData>
  inline void
  Mesh<VertData, TriData>::for_verts(std::function<void(VertData & v)> func) {
    for (auto & v : verts)
      func(v);
  }
