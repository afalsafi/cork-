...
namespace corkpp {
  ...
  enum EdgeRemeshOperation { EDGE_SPLIT, EDGE_COLLAPSE, EDGE_NOTHING };
  ...
  template <class VertData, class TriData>
  void
  Mesh<VertData, TriData>::dequeue(std::set<std::pair<double, Eptr>> & queue,
                                   Eptr edge) {
  ...
  inline void Mesh<VertData, TriData>::edgeNeighborhood(
      Eptr edge, std::function<void(VertData & v0, VertData & v1)> once,
      std::function<void(VertData & v0, VertData & v1, VertData & vopp,
                         TriData & t)>
          each_tri) {
  ...
  double Mesh<VertData, TriData>::computeEdgeScore(Eptr edge) {
    ...
    edgeNeighborhood(edge,
                     [&edge_length](VertData & v0, VertData & v1) {  // once
                       edge_length = len(v1.pos - v0.pos);
                     },
                     [&min_angle, &max_angle](VertData & v0, VertData & v1,
                                              VertData & vopp,
                                              TriData &) {  // per tri
  ...
  inline void remove(ShortVec<T, LEN> & vec, T val) {
  ...
  // IF THE VALUE OCCURS
  template <class T, uint LEN>
  inline void maybe_erase(ShortVec<T, LEN> & vec, T val) {
    uint N = vec.size();
    for (uint i = 0; i < N; i++) {
      if (vec[i] == val) {
  ...
  template <class VertData, class TriData>
  inline void Mesh<VertData, TriData>::populateTriFromTopoTri(Tptr tri) {
  ...
  template <class VertData, class TriData>
  Eptr
  Mesh<VertData, TriData>::allocateRemeshEdge(RemeshScratchpad & scratchpad) {
  ...
  template <class VertData, class TriData>
  void
  Mesh<VertData, TriData>::deallocateRemeshEdge(RemeshScratchpad & scratchpad,
                                                Eptr e) {
  ...
  template <class VertData, class TriData>
  inline void Mesh<VertData, TriData>::merge_tris(uint tid_result, uint tid0,
                                                  uint tid1) {
    tris[tid_result].data.merge(tris[tid0].data, tris[tid1].data);
  }
  template <class VertData, class TriData>
  inline void Mesh<VertData, TriData>::split_tris(uint t0ref, uint t1ref,
                                                  uint t_orig_ref) {
  ...
  */
  ...
  struct PtrRemap {
    ...
    void set(T * key, T * val) { remap[key] = val; }
  ...
  template <class VertData, class TriData>
  void
  Mesh<VertData, TriData>::edgeCollapse(RemeshScratchpad & scratchpad,
                                        Eptr e_collapse,
    ...
    for (Tptr t : v0->tris) {
      if (t->edges[0] == e_collapse || t->edges[1] == e_collapse ||
          t->edges[2] == e_collapse)
        continue;
  ...
  */
  ...
  template <class VertData, class TriData>
  void Mesh<VertData, TriData>::edgeSplit(RemeshScratchpad & scratchpad,
                                          Eptr e_split) {
    /*
