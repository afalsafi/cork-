...
namespace corkpp {
  ...
  class Vec3 {
   ...
   public:  // constructors
    inline Vec3() {}
    inline Vec3(const N & n0, const N & n1, const N & n2)
        : x(n0), y(n1), z(n2) {}
    // allows implicit conversion based on generics...
    template <class T>
  ...
  template <class N>
  inline N det(const Vec4<N> & v0, const Vec4<N> & v1, const Vec4<N> & v2,
               const Vec4<N> & v3);
  // +---------------------------------
  // collapsing measurements
  template <class N>
  ...
  template <class N>
  inline Vec2<N> max(const Vec2<N> & lhs, const Vec2<N> & rhs) {
    return Vec2<N>(std::max(lhs.x, rhs.x), std::max(lhs.y, rhs.y));
  }
  template <class N>
  inline Vec2<N> min(const Vec2<N> & lhs, const Vec2<N> & rhs) {
    return Vec2<N>(std::min(lhs.x, rhs.x), std::min(lhs.y, rhs.y));
  }
  // +---------------------------------
  // dimension logic for projections
  template <class N>
  ...
  template <class N>
  inline Vec3<N> max(const Vec3<N> & lhs, const Vec3<N> & rhs) {
    return Vec3<N>(std::max(lhs.x, rhs.x), std::max(lhs.y, rhs.y),
                   std::max(lhs.z, rhs.z));
  }
  template <class N>
  inline Vec3<N> min(const Vec3<N> & lhs, const Vec3<N> & rhs) {
    return Vec3<N>(std::min(lhs.x, rhs.x), std::min(lhs.y, rhs.y),
                   std::min(lhs.z, rhs.z));
  }
  // +---------------------------------
  // dimension logic for projections
  ...
  template <class N>
  inline Vec3<N> cross(const Vec3<N> & lhs, const Vec3<N> & rhs) {
    return Vec3<N>(lhs.y * rhs.z - lhs.z * rhs.y, lhs.z * rhs.x - lhs.x * rhs.z,
  ...
  template <class N>
  inline Vec4<N> max(const Vec4<N> & lhs, const Vec4<N> & rhs) {
    return Vec4<N>(std::max(lhs.x, rhs.x), std::max(lhs.y, rhs.y),
                   std::max(lhs.z, rhs.z), std::max(lhs.w, rhs.w));
  }
  template <class N>
  inline Vec4<N> min(const Vec4<N> & lhs, const Vec4<N> & rhs) {
    return Vec4<N>(std::min(lhs.x, rhs.x), std::min(lhs.y, rhs.y),
  ...
  template <class N>
  inline uint maxDim(const Vec4<N> & vec) {
    return (vec.x >= vec.y) ? ((vec.x >= vec.z) ? ((vec.x >= vec.w) ? 0 : 3)
                                                : ((vec.z >= vec.w) ? 2 : 3))
  ...
  template <class N>
  inline Vec3<N> proj(uint dim, const Vec4<N> & vec) {
    return Vec3<N>(vec.v[(dim + 1) % 4], vec.v[(dim + 2) % 4],
                   vec.v[(dim + 3) % 4]);
  }
  // +---------------------------------
  // more sophisticated vector arithmetic
  ...
  inline N det(const Vec4<N> & v0, const Vec4<N> & v1, const Vec4<N> & v2,
               const Vec4<N> & v3) {
    N xy01 = v0.x * v1.y - v0.y * v1.x;
    N yz01 = v0.y * v1.z - v0.z * v1.y;
    N xz01 = v0.x * v1.z - v0.z * v1.x;
    N yw01 = v0.y * v1.w - v0.w * v1.y;
    N xw01 = v0.x * v1.w - v0.w * v1.x;
    N zw01 = v0.z * v1.w - v0.w * v1.z;

    N xy23 = v2.x * v3.y - v2.y * v3.x;
    N yz23 = v2.y * v3.z - v2.z * v3.y;
    N xz23 = v2.x * v3.z - v2.z * v3.x;
    N yw23 = v2.y * v3.w - v2.w * v3.y;
    N xw23 = v2.x * v3.w - v2.w * v3.x;
    N zw23 = v2.z * v3.w - v2.w * v3.z;

    return xy01 * zw23 - xz01 * yw23 + xw01 * yz23 + yz01 * xw23 - yw01 * xz23 +
           zw01 * xy23;
  }
  // +---------------------------------
  // collapsing measurements
