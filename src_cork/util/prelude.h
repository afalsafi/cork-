...
#define ENSURE(STATEMENT)                                               \
  {                                                                     \
    if (!(STATEMENT)) {                                                 \
      std::cerr << "ENSURE FAILED at " << __FILE__ << ", line #" << __LINE__ \
                << ":\n"                                                \
                << "    " << #STATEMENT << std::endl;                   \
      err() << "ENSURE FAILED at " << __FILE__ << ", line #" << __LINE__ \
            << ":\n"                                                    \
            << "    " << #STATEMENT << std::endl;                       \
      exit(1);                                                          \
    }                                                                   \
...
#define CORK_ERROR(message)                                             \
  {                                                                     \
    std::cerr << "error at " << __FILE__ << ", line #" << __LINE__ << ": " \
              << (message) << std::endl;                                \
    err() << "error at " << __FILE__ << ", line #" << __LINE__ << ": "  \
          << (message) << std::endl;                                    \
  }
#endif  // CORK_ERROR
...
#define MARKER(message)                                                 \
  {                                                                     \
    std::cout << "marker at " << __FILE__ << ", line #" << __LINE__ << ": " \
              << (message) << std::endl;                                \
  }
#endif  // MARKER
