...
namespace corkpp {
  ...
  template <class T, uint LEN>
  inline T * ShortVec<T, LEN>::allocData(uint space, uint & allocated) {
    T * result;
    if (space <= LEN) {
      allocated = LEN;
  ...
  template <class T, uint LEN>
  inline void ShortVec<T, LEN>::deallocData(T * data_ptr, uint allocated) {
    // if(LEN == 2) std::cout << "        Deallocing: " << data_ptr <<
    // std::endl;
    if (allocated <= LEN)
  ...
  template <class T, uint LEN>
  inline void ShortVec<T, LEN>::constructRange(T * array, int begin, int end) {
    for (int i = begin; i < end; i++)
      new (&(array[i])) T();
  }
  template <class T, uint LEN>
  inline void ShortVec<T, LEN>::copyConstructRange(T * src, T * dest, int begin,
                                                   int end) {
  ...
  template <class T, uint LEN>
  inline void ShortVec<T, LEN>::destructRange(T * array, int begin, int end) {
    for (int i = begin; i < end; i++)
      (&(array[i]))->~T();
  }
  ...
  template <class T, uint LEN>
  inline void ShortVec<T, LEN>::resizeHelper(uint newsize) {
  ...
  template <class T, uint LEN>
  inline ShortVec<T, LEN>::ShortVec(uint size) : user_size(size) {
    data = allocData(user_size, internal_size);
    constructRange(data, 0, user_size);
  }
  template <class T, uint LEN>
  inline ShortVec<T, LEN>::ShortVec(uint size, const T & fill_val)
      : user_size(size) {
    data = allocData(user_size, internal_size);
    // then fill
  ...
  template <class T, uint LEN>
  inline ShortVec<T, LEN>::ShortVec(const ShortVec<T, LEN> & cp)
  ...
  template <class T, uint LEN>
  inline ShortVec<T, LEN>::~ShortVec() {
    destructRange(data, 0, user_size);
    deallocData(data, internal_size);
  }

  template <class T, uint LEN>
  inline ShortVec<T, LEN> & ShortVec<T, LEN>::
  operator=(const ShortVec<T, LEN> & vec) {
  ...
  template <class T, uint LEN>
  inline void ShortVec<T, LEN>::resize(uint newsize) {
  ...
  template <class T, uint LEN>
  inline void ShortVec<T, LEN>::push_back(const T & datum) {
    uint i = user_size;
  ...
  template <class T, uint LEN>
  inline void ShortVec<T, LEN>::erase(const T & val) {
    for (uint i = 0; i < user_size; i++) {
      if (data[i] == val) {
