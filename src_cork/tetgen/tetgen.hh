...
namespace corkpp {
  ...
  //   Visual C++ 2003 -- 2008 (Visual C++ 7.1 - 9) doesn't ship with this
  //   header file. In such case, we can define them by ourself.
  // Update (learned from Stack Overflow): Visual Studio 2010 and Visual C++
  // 2010
  //   Express both have stdint.h

  // The following piece of code was provided by Steven Johnson (MIT). Define
  // the
...
#include <stdint.h>
  ...
  // tetgenio //
  //                                                                           //
  // A structure for transferring data into and out of TetGen's mesh
  // structure,// 'tetgenmesh' (declared below). //
  //                                                                           //
  ...
  // // ghted Delaunay) tetrahedralization, or a constrained (Delaunay)
  // tetrahed- // ralization, or a quality tetrahedral mesh. //
  //                                                                           //
  ...
  // // facets, tetrahedra, and so forth. It contains functions to read and
  // write // (input and output) files of TetGen as well as other supported mesh
  // files. //
  //                                                                           //
  ...
  // // test. Each of these tests is performed by evaluating the sign of a
  // deter- // minant of a matrix whose entries are the coordinates of these
  // points.  If // the computation is performed by using the floating-point
  // numbers, e.g.,   // the single or double precision numbers in C/C++,
  // roundoff error may cause // an incorrect result. This may either lead to a
  // wrong result or eventually // lead to a failure of the program.  Computing
  // the predicates exactly will  // avoid the error and make the program
  // robust.                              //
  //                                                                           //
  ...
  REAL orient4d(REAL * pa, REAL * pb, REAL * pc, REAL * pd, REAL * pe, REAL ah,
                REAL bh, REAL ch, REAL dh, REAL eh);
  ...
  class tetgenmesh {
   ...
   public:
    ...
    // // tetrahedron contains pointers to its vertices and adjacent tetrahedra.
    // A  // vertex stores its x-, y-, and z-coordinates, and a pointer to a
    // tetrahed- // ron containing it. Both tetrahedra and vertices may contain
    // user data.    //
    //                                                                           //
    ...
    // boundary.  It has the  // property that every face is shared by exactly
    // two tetrahedra.             //
    //                                                                           //
    ...
    // integer, in the   // range from 0 to 11. v is called the ``version'' of a
    // tetrahedron, it rep- // resents a directed edge of a specific face of the
    // tetrahedron.            //
    //                                                                           //
...
#define fastlookup(pool, index)                                                \
  (void *)((pool)->toparray[(index) >> (pool)->log2objectsperblock] +          \
    ...
    class badface {
     public:
      ...
      badface * nextitem;
      badface()
          : key(0), forg(0), fdest(0), fapex(0), foppo(0), noppo(0),
            nextitem(0) {}
    };
    ...
    // // et al [*].  Hence the point-in-sphere test never returns a zero. The
    // idea // is to perturb the weights of vertices in the fourth dimension.
    // TetGen    // uses the indices of the vertices decide the amount of
    // perturbation. It is // implemented in the routine insphere_s().
    //                                                                           //
    // The routine tri_edge_test() determines whether or not a triangle and an
    // // edge intersect in 3D. If they intersect, their intersection type is
    // also  // reported. This test is a combination of n 3D orientation tests
    // (n is bet- // ween 3 and 9). It uses the robust orient3d() test to make
    // the branch dec- // isions.  The routine tri_tri_test() determines whether
    // or not two triang- // les intersect in 3D. It also uses the robust
    // orient3d() test.             //
    //                                                                           //
    ...
    // // They are so far done by the default floating-point arithmetics which
    // are  // non-robust. They should be improved in the future. //
    //                                                                           //
    ///////////////////////////////////////////////////////////////////////////////
    ...
    int tri_edge_2d(point, point, point, point, point, point, int, int *,
                    int *);
    ...
    bool circumsphere(REAL *, REAL *, REAL *, REAL *, REAL * cent,
                      REAL * radius);
    ...
    // tet- // rahedron which is removed immediately by the followed 3-to-2
    // flip.  More  // generally, a n-to-m flip, where n > 3, m = (n - 2) * 2,
    // which removes an  // edge can be done by first performing a sequence of
    // (n - 3) 2-to-3 flips   // followed by a 3-to-2 flip. //
    //                                                                           //
    ...
    // // algorithm to insert a vertex. It works for arbitrary
    // tetrahedralization,  // either Delaunay, or constrained Delaunay, or
    // non-Delaunay.                //
    //                                                                           //
    ///////////////////////////////////////////////////////////////////////////////
    ...
    // arbitr- // ary tetrahedron in DT, it finds the destination by visit one
    ...
    // Geometry,// 274-283, 1996.  It first randomly samples several tetrahedra
    // in the DT    // and then choosing the closet one to start walking. //
    ...
    // // the points are pre-sorted so that the nearby points in space have
    // nearby  // indices, then adding the points in this order. They sorted the
    ...
    // // "Compact Hilbert Indices", Technical Report CS-2006-07, Computer
    // Science, // Dalhousie University, 2006 (the Section 2). My implementation
    // also refer- // enced Steven Witham's implementation of "Hilbert walk"
    ...
    void interecursive(shellface ** subfacearray, int arraysize, int axis, REAL,
                       REAL, REAL, REAL, REAL, REAL, int * internum);
    ...
    // // unay tetrahedralization (DT) that is constrained to respect the
    // boundary  // of a 3D PLC (domain). In a CDT of a 3D PLC, every vertex or
    // edge of the   // PLC is also a vertex or an edge of the CDT, every
    // polygon of the PLC is a // union of triangles of the CDT. A crucial
    // difference between a CDT and a   // DT is that triangles in the PLC's
    // polygons are not required to be locally // Delaunay, which frees the CDT
    // to better respect the PLC's polygons. CDTs  // have optimal properties
    // similar to those of DTs.                          //
    //                                                                           //
    // Steiner Points and Steiner CDTs. It is known that even a simple 3D polyh-
    // // edron may not have a tetrahedralization which only uses its own
    // vertices. // Some extra points, so-called "Steiner points" are needed in
    // order to form // a tetrahedralization of such polyhedron.  It is true for
    // tetrahedralizing // a 3D PLC as well. A Steiner CDT of a 3D PLC is a CDT
    // containing Steiner   // points. The CDT algorithms of TetGen in general
    // create Steiner CDTs.      // Almost all of the Steiner points are added
    // in the edges of the PLC. They  // guarantee the existence of a CDT of the
    // modified PLC.                     //
    //                                                                           //
    // The routine constraineddelaunay() starts from a DT of the vertices of a
    // // PLC and creates a (Steiner) CDT of the PLC (including Steiner points).
    // It // is constructed by two steps, (1) segment recovery and (2) facet
    // (polygon) // recovery. Each step is accomplished by its own algorithm. //
    //                                                                           //
    ...
    // Constr- // ained Delaunay Tetrahedralizations," In Proceedings of the
    // 14th Internat- // ional Meshing Roundtable, 147--163, 2005.  It adds
    // Steiner points into    // non-Delaunay segments until all subsegments
    // appear together in a DT. The  // running time of this algorithm is
    // proportional to the number of added     // Steiner points. //
    //                                                                           //
    ...
    // // Constrained Delaunay Tetrahedralization," International Journal for
    // Numer-// ical Methods in Engineering, 85:1341-1364, 2011, and the flip
    // algorithm   // of Shewchuk, J. "Updating and Constructing Constrained
    // Delaunay and       // Constrained Regular Triangulations by Flips." In
    ...
    // // However, a facet with non-coplanar vertices might cause the  additions
    // of // Steiner points. It is discussed in the paper of Si, H., and
    // Shewchuk, J.,// "Incrementally Constructing and Updating Constrained
    // Delaunay             // Tetrahedralizations with Finite Precision
    // Coordinates." In Proceedings of // the 21th International Meshing
    // Roundtable, 2012.                          //
    //                                                                           //
    ...
    //                                                                           //
    // The cavity re-triangulation algorithm is implemented by three
    // subroutines,// delaunizecavity(), fillcavity(), and carvecavity(). Since
    // it may fail due // to non-coplanar vertices, the subroutine
    // restorecavity() is used to rest- // ore the original cavity. //
    //                                                                           //
    ...
    // // missing region. In order to avoid inserting Steiner points very close
    // to  // existing segments.  The classical encroachment rules of the
    // Delaunay      // refinement algorithm are used to choose the Steiner
    // points.               //
    //                                                                           //
    // The routine constrainedfacets() does the facet recovery by using either
    // // the cavity re-triangulation algorithm (default) or the flip algorithm.
    // It // results a CDT of the (modified) PLC (including Steiner points). //
    //                                                                           //
    ///////////////////////////////////////////////////////////////////////////////
    ...
    bool formcavity(triface *, arraypool *, arraypool *, arraypool *,
    ...
    int insertpoint_cdt(point, triface *, face *, face *, insertvertexflags *,
                        ...
                        arraypool *, arraypool *);
    void refineregion(face &, arraypool *, arraypool *, arraypool *,
                      arraypool *, arraypool *, arraypool *);
    ...
    int getvertexstar(int, point searchpt, arraypool *, arraypool *,
                      arraypool *);
    ...
    // insert  // new Steiner points to achieve this property. The questions are
    // (1) how to // choose the Steiner points? and (2) how to insert them? //
    //                                                                           //
    ...
    // ACM   // Symposium on Computational Geometry, 86-95, 1998.  It guarantees
    // that all // tetrahedra of the output mesh have a "radius-edge ratio"
    // (equivalent to   // the minimal face angle) bounded. However, it does not
    // remove slivers, a   // type of very flat tetrahedra which can have no
    // small face angles but have // very small (and large) dihedral angles.
    // Moreover, it may not terminate if // the input PLC contains "sharp
    // features", e.g., two edges (or two facets)  // meet at an acute angle (or
    // dihedral angle).                               //
    //                                                                           //
    ...
    // algorithm is  // described in Si, H., "Adaptive Constrained Delaunay Mesh
    ...
    int splitsegment(face * splitseg, point encpt, REAL, point, point, int,
                     int);
    ...
    int splitsubface(face * splitfac, point, point, int qflag, REAL * ccent,
                     int);
  ...
  //                                                                           //
  // 'in' is an object of 'tetgenio' which contains a PLC you want to
  // tetrahed-// ralize or a previously generated tetrahedral mesh you want to
  // refine.  It // must not be a NULL. 'out' is another object of 'tetgenio'
  // for storing the // generated tetrahedral mesh. It can be a NULL. If so, the
  // output will be   // saved to file(s). If 'bgmin' != NULL, it contains a
  // background mesh which // defines a mesh size function. //
  //                                                                           //
  ///////////////////////////////////////////////////////////////////////////////
  ...
  inline tetgenmesh::tetrahedron tetgenmesh::encode2(tetrahedron * ptr,
                                                     int ver) {
